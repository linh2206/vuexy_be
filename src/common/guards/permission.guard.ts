...
export class PermissionGuard implements CanActivate {
    ...
    private async verifyPermission(data: { req: Request; permission: string; params: any }) {
        try {
            ...
            const email = data.req.headers['_email'].toString(); // sau khi qua authMiddleware thì đã add _kId vào body
    ...
    private async getUser(email: string): Promise<AccountEntity> {
        const key = `userData:${email}`;
        const cached = await this.cacheService.getJson(key);
        if (cached) return cached;

        const entityManager = this.dataSource.manager;
        const user = await entityManager.findOne(AccountEntity, { where: { email: email }, select: ['id', 'email'] });
        return user;
    }
}
